...
function event()
  ...
  local res, err = con:query("SELECT v.visit_date, v.user_id AS patient_id, v.facility_id, string_agg(r.value::text, ', '::text ORDER BY (r.value::text)) AS reasons FROM visit v JOIN visit_reasons vr ON v.id = vr.visit_id JOIN reason r ON vr.reason_id = r.id WHERE v.active = true GROUP BY v.id, v.visit_date, v.user_id, v.facility_id ORDER BY v.visit_date DESC;")
  if not res then
